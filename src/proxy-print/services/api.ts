import { Card, APIResponse } from '@/proxy-print/types';
...
class OPTCGAPI {
  ...
  private isOptcgThrottled(): boolean {
    ...
    if (Date.now() > this.optcgThrottledUntil) {
      this.optcgThrottled = false;
      return false;
    }
    return true;
  }

  /** Browser: same-origin proxy (avoids CORS / Safari "TypeError: Load failed"). Server: direct OPTCG. */
  private optcgClientFetch(relativePath: string): string {
    const norm = relativePath.replace(/^\//, "");
    if (typeof window !== "undefined") {
      return `/api/optcg-proxy?path=${encodeURIComponent(norm)}`;
    }
    return `${this.baseURL}/${norm}`;
  }

  private limitlessClientFetch(cardId: string): string {
    if (typeof window !== "undefined") {
      return `/api/limitless-proxy?cardId=${encodeURIComponent(cardId)}`;
    }
    return `${this.limitlessBaseURL}/cards/${cardId}`;
  }
  ...
  // OPTCG API set ID mapping for combined/special sets
  private static readonly OPTCG_SET_ID_MAP: Record<string, string> = {
    'OP14': 'OP14-EB04',
    'EB04': 'OP14-EB04',
  };

  // Check if set should use Limitless API (ST sets, PRB promo sets, and individual P-xxx cards)
  // OPTCG API now covers: OP01-OP14 (OP14 combined with EB04), EB01-EB03
  private shouldUseLimitlessAPI(setCode: string): boolean {
    if (!setCode) return false;
    const upperCode = setCode.toUpperCase().replace(/-/g, '');
    
    // ST sets always use Limitless (OPTCG API doesn't have ST sets)
    if (upperCode.startsWith('ST')) return true;
    
    // PRB (Promo) sets use Limitless (OPTCG PRB data uses original set card IDs)
    if (upperCode.startsWith('PRB')) return true;
    
    // P (Promo) format - single letter P indicates individual promo cards
    if (upperCode === 'P' || upperCode.startsWith('P-')) return true;
    
    // OP sets: OP15+ use Limitless (OPTCG API has OP01-OP14)
    const opMatch = upperCode.match(/^OP(\d+)$/);
    if (opMatch) {
      const opNumber = parseInt(opMatch[1]);
      if (opNumber >= 15) return true;
    }
    
    // EB sets: EB05+ use Limitless (OPTCG API has EB01-EB04 via combined set)
    const ebMatch = upperCode.match(/^EB(\d+)$/);
    if (ebMatch) {
      const ebNumber = parseInt(ebMatch[1]);
      if (ebNumber >= 5) return true;
    }
  ...
  private async getCardFromLimitless(cardId: string): Promise<any | null> {
    try {
      ...
      const response = await fetch(this.limitlessClientFetch(cardId));
  ...
  // Resolve set code to OPTCG API set ID (handles dash format and combined sets)
  private resolveOptcgSetId(setCode: string): string {
    const noDash = setCode.toUpperCase().replace(/-/g, '');
    
    // Check combined set mapping (e.g., OP14 → OP14-EB04)
    const mapped = OPTCGAPI.OPTCG_SET_ID_MAP[noDash];
    if (mapped) return mapped;
    
    // Ensure dash format: OP01 → OP-01, EB03 → EB-03, PRB01 → PRB-01
    const match = noDash.match(/^([A-Z]{2,3})(\d{2,})$/);
    if (match) {
  ...
  private async getCardsFromSet(setId: string): Promise<any[]> {
    const resolvedId = this.resolveOptcgSetId(setId);
    const cacheKey = `set_${resolvedId}`;
    const cached = this.cache.get(cacheKey);
    
    if (cached) {
      console.log(`Using cached cards for set ${resolvedId}`);
      return cached;
    }

    try {
      if (this.isOptcgThrottled()) {
        console.log(`OPTCG API throttled, skipping set ${setId}`);
        return [];
      }

      console.log(`Fetching cards from set ${resolvedId} (input: ${setId})...`);
      const response = await fetch(this.optcgClientFetch(`sets/${resolvedId}/`));
      
      if (!response.ok) {
        if (response.status === 429) {
          const body = await response.text().catch(() => '');
          const match = body.match(/available in (\d+)/);
          const retrySeconds = match ? parseInt(match[1]) : 300;
          this.optcgThrottled = true;
          this.optcgThrottledUntil = Date.now() + retrySeconds * 1000;
          console.warn(`OPTCG API throttled for ${retrySeconds}s`);
          console.warn(`OPTCG set ${setId}: rate limited (429)`);
        } else {
  ...
  private async getAllSets(): Promise<any[]> {
    ...
    try {
      if (this.isOptcgThrottled()) {
        console.log('OPTCG API throttled, skipping getAllSets');
        return [];
      }

      console.log('Fetching all sets from OPTCG API...');
      const response = await fetch(this.optcgClientFetch("allSets/"));
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          this.optcgThrottledUntil = Date.now() + retrySeconds * 1000;
          console.warn(`OPTCG API throttled for ${retrySeconds}s`);
          console.warn(`OPTCG allSets: rate limited (429)`);
        } else {
    ...
    try {
      ...
      if (setCode) {
        ...
        if (this.shouldUseLimitlessAPI(setCode)) {
          ...
          let setCards = await this.getCardsFromSet(setCode);

          // Production safety: if OPTCG throttling makes setCards empty,
          // fall back to local repo index so deck resolution and modal still work.
          if (setCards.length === 0) {
            try {
              ...
              const setCodeClean = (setCode || "")
                .toUpperCase()
                .replace(/-/g, "");
              const wantedNumber = baseNumber + variantSuffix; // e.g. "020_p1" or "020"
              const wantedNumberLower = wantedNumber.toLowerCase();

              const localMatches = this.fullCardListCache
                .filter((card) => {
                  const cardSet = (card.set_code || "").toUpperCase().replace(/-/g, "");
                  if (cardSet !== setCodeClean) return false;

                  const cardNumber = (card.number || "").toString();
                  if (!cardNumber) return false;
                  return cardNumber.toLowerCase() === wantedNumberLower;
                })
                // Standard first when no variant suffix requested
                .sort((a, b) => {
                  const aa = a.id?.toLowerCase().includes("_aa") ? 1 : 0;
                  const bb = b.id?.toLowerCase().includes("_aa") ? 1 : 0;
                  return aa - bb;
                });

              if (localMatches.length > 0) {
                searchResults = localMatches.slice(0, 5);
                console.log(
                  `Fallback(local): resolved ${searchResults.length} cards for set ${setCode} number ${cardNumber}`,
                );
                return searchResults;
              }
            } catch {
              // Keep original behavior; later fallbacks may still handle it.
            }
          }
          
          // For combined sets (e.g., OP14-EB04), filter to only the requested set's cards
          const noDashSet = setCode.toUpperCase().replace(/-/g, '');
          if (OPTCGAPI.OPTCG_SET_ID_MAP[noDashSet]) {
            setCards = setCards.filter((card: any) => {
              const cardSetId = (card.card_set_id || '').toUpperCase();
              return cardSetId.startsWith(noDashSet);
            });
            console.log(`Filtered combined set to ${noDashSet}: ${setCards.length} cards`);
          }
  ...
  /**
   * OPTCG sometimes returns the same `card_set_id` for standard and alt-art rows; the only
   * difference is `card_image` filename (e.g. …/OP10-005.png vs …/OP10-005_aa.png).
   * op2 merges the catalog with Map(id) — duplicate ids collapse and Alternative Arts breaks.
   */
  private deriveCardIdFromOptcgImageUrl(
    imageUrl: string,
    cardSetId: string,
  ): string | null {
    if (!imageUrl || !cardSetId) return null;
    const setStem = cardSetId.split("_")[0].trim();
    if (!setStem) return null;
    const setStemUp = setStem.toUpperCase();

    const m = imageUrl.match(
      /([A-Za-z]{2,3}\d{2}-\d{3})(_[A-Za-z0-9]+)?\.(?:png|jpg|jpeg|webp)/i,
    );
    if (!m?.[1]) return null;
  ...
  private normalizeCard(card: any): Card {
    ...
    } else if (!requestedVariantSuffix && imageUrl && !String(cardId).includes("_")) {
      const derivedId = this.deriveCardIdFromOptcgImageUrl(imageUrl, cardSetId);
      if (derivedId) {
        cardId = derivedId;
        imageUrl = `https://en.onepiece-cardgame.com/images/cardlist/card/${derivedId}.png`;
      }
    }
    ...
    const idNum = String(cardId).match(/^[A-Za-z]{2,3}\d{2}-(\d{3})(_[A-Za-z0-9]+)?$/i);
    const numberWithVariant = idNum
      ? `${idNum[1]}${(idNum[2] || "").toLowerCase()}`
      : requestedVariantSuffix
        ? `${baseNumber}${requestedVariantSuffix}`
        : baseNumber;
    
    // Derive actual set code from card_set_id (e.g., "OP14-096" → "OP14")
    // This handles combined sets like OP14-EB04 where set_id doesn't reflect the card's actual set
    const derivedSetCode = cardSetId?.split('-')[0] || card.set_id;
    
    const normalizedCard: Card = {
      id: cardId,
      name: card.card_name,
      set_code: derivedSetCode,
  ...
  // Get all cards for search and filtering (same strategy as op/src/services/api.ts: full OPTCG catalog)
  async getAllCardsForSearch(): Promise<Card[]> {
    ...
    const loadLocalIndex = async (): Promise<Card[]> => {
    ...
    let fromLocal: Card[] = [];
    try {
      const [localCards, allSets] = await Promise.all([
        loadLocalIndex(),
        this.getAllSets(),
      ]);
      fromLocal = localCards;

      if (!allSets.length) {
        console.warn("No sets from OPTCG (throttle or error)");
        if (fromLocal.length > 0) {
          this.fullCardListCache = fromLocal;
          this.fullCardListCacheTime = Date.now();
          this.initFuseInstance(fromLocal);
          console.warn(
            `Search using local index only (${fromLocal.length} cards) until OPTCG is reachable`,
          );
          return fromLocal;
        }
        return [];
      ...
      console.log(
        "Fetching all cards for search (OPTCG parallel batches, same as op)…",
        fromLocal.length ? `merging +${fromLocal.length} local index extras after API` : "",
      );
      ...
      for (let i = 0; i < allSets.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (set: { set_id?: string }) => {
        ...
        batchResults.forEach((cards) => fromApi.push(...cards));
      }
      ...
      this.fullCardListCache = merged;
      this.fullCardListCacheTime = Date.now();
      this.initFuseInstance(merged);
      console.log(
        `Loaded ${merged.length} cards for search (OPTCG ${fromApi.length}, +local-only ${merged.length - fromApi.length})`,
      );
      return merged;
    } catch (error) {
  ...
  /** Strip parallel / alt-art suffixes for ID parsing (e.g. OP13-005_p1 → OP13-005). */
  private splitCardIdVariant(upper: string): { base: string; variantSuffix: string } {
    const m = upper.match(
      /^([A-Z]{2,3}\d{1,2}-\d{1,3})((_P\d+|_AA|_SP|_MANGA)+)$/i,
    );
    if (m) return { base: m[1].toUpperCase(), variantSuffix: m[2].toUpperCase() };
    return { base: upper, variantSuffix: "" };
  }

  // Check if query is a card ID format (e.g., OP13-046, ST21-001, OP13-005_p1, P-107, p-)
  private isCardIdFormat(query: string): boolean {
    const trimmed = query.trim();
    if (/^P-?\d{0,3}(_[a-z0-9]+)?$/i.test(trimmed)) return true;
    const u = trimmed.toUpperCase();
    const { base } = this.splitCardIdVariant(u);
    return /^[A-Za-z]{2,3}\d{1,2}-?\d{0,3}$/i.test(base);
  }

  // Parse card ID format to get set code and number
  private parseCardId(query: string): {
    setCode: string;
    number: string | null;
    fullId: string;
    ...
    const { base, variantSuffix } = this.splitCardIdVariant(trimmed);

    // Promo kart formatı: P-107, P-029, P-, p-
    const promoMatch = base.match(/^P-?(\d{0,3})$/i);
    if (promoMatch) {
      if (promoMatch[1]) {
        const cardNum = promoMatch[1].padStart(3, "0");
        const fullId = `P-${cardNum}${variantSuffix}`;
        return {
          setCode: "PRB",
          number: cardNum,
          fullId,
      ...
      return {
        setCode: "PRB",
        number: null,
        fullId: `P${variantSuffix}`,
        variantSuffix,
      };
    }

    const match = base.match(/^([A-Za-z]{2,3})(\d{1,2})-?(\d{0,3})$/i);
    if (match) {
      const setPrefix = match[1].toUpperCase();
      const setNum = match[2].padStart(2, "0");
      const cardNumRaw = match[3];
      const cardNum = cardNumRaw ? cardNumRaw.padStart(3, "0") : null;
      const fullId = cardNum
        ? `${setPrefix}${setNum}-${cardNum}${variantSuffix}`
        : `${setPrefix}${setNum}${variantSuffix}`;
      return {
  ...
  async searchCardsAutocomplete(query: string, limit: number = 10): Promise<Card[]> {
    ...
    // If the user typed an ID-like query with an unknown suffix (e.g. OP14-042_rajdyck),
    // treat it as invalid and don't fall back to fuzzy search (it can surface wrong cards).
    {
      const upper = query.trim().toUpperCase();
      const m = upper.match(/^([A-Z]{2,3})(\d{1,2})-?(\d{1,3})_([A-Z0-9]+)$/i);
      if (m) {
        const suffix = `_${m[4]}`;
        const isAllowed =
          /^_P\d+$/i.test(suffix) ||
    ...
    // Direct substring: name, card id, set code (e.g. "OP13-005" when cache is partial)
    ...
    nameMatches.sort((a, b) => {
      ...
      const aIdStarts = aid.startsWith(queryLower) ? 0 : 1;
      const bIdStarts = bid.startsWith(queryLower) ? 0 : 1;
  ...
  async getCardVariants(cardId: string): Promise<Card[]> {
    ...
    if (this.shouldUseLimitlessAPI(setCode)) {
      ...
      const variants: Card[] = [];
      const variantSuffixes = ['', '_p1', '_p2', '_p3', '_p4', '_aa'] as const;
      ...
      for (const suffix of variantSuffixes) {
        ...
        const variantCard: Card = {
          ...
          flavor_text: '',
          artist: '',
          variant: suffix === '_aa' ? 'alternate-art' : suffix ? 'parallel' : 'standard',
          variantLabel:
            suffix === '_aa'
              ? 'AA'
              : suffix
    ...
    // Cloudflare/proxy fallback: if catalog is local-only, variants may collapse to a single base card.
    // Generate common official image variants so the art switcher remains usable.
    if (variants.length <= 1) {
      const fallbackSuffixes = ["", "_p1", "_p2", "_p3", "_p4", "_aa"] as const;
      const generated = fallbackSuffixes.map((suffix) => {
        const id = `${setCode}-${cardNumber}${suffix}`;
        const imageUrl = `https://en.onepiece-cardgame.com/images/cardlist/card/${id}.png`;
        return {
          id,
          name: variants[0]?.name || `Card ${id}`,
          set_code: setCode,
          set_name: variants[0]?.set_name || setCode,
          number: `${cardNumber}${suffix}`,
          rarity: variants[0]?.rarity || "Unknown",
          image_uris: { full: imageUrl, large: imageUrl, small: imageUrl },
          colors: variants[0]?.colors || [],
          cost: variants[0]?.cost,
          type: variants[0]?.type || "Unknown",
          subtypes: variants[0]?.subtypes || [],
          characteristics: variants[0]?.characteristics || [],
          text: variants[0]?.text || "",
          flavor_text: variants[0]?.flavor_text || "",
          artist: variants[0]?.artist || "",
          variant: suffix === "_aa" ? "alternate-art" : suffix ? "parallel" : "standard",
          variantLabel:
            suffix === "_aa"
              ? "AA"
      ...
      console.log(
        `Fallback-generated ${generated.length} variants for ${baseCardId} (cache variants: ${variants.length})`,
      );
      return generated;
    }
