...
import { DeckCard, PrintSettings } from '@/proxy-print/types';
import { usePdfGenerationState, PdfGenerationState } from '@/proxy-print/hooks/usePdfGenerationState';
...
export const PdfGeneratorClient = forwardRef<PdfGeneratorClientRef, PdfGeneratorClientProps>(
  ({ cards, printSettings, onProgress, onSuccess, onError, abortSignal }, ref) => {
    ...
    const preloadPdfImages = useCallback(async (cardData: DeckCard[]): Promise<Map<string, string>> => {
      ...
      const { getImageAsDataUri } = await import('@/proxy-print/utils/imageDataUri');
      const { toAbsoluteUrl } = await import('@/proxy-print/utils/url');
